...
function weaponProfile(weaponId: string): WeaponProfile {
  if (weaponId === "safety_cannon") {
    return { label: "safety cannon", range: 8.2, cooldownScale: 1.12, damageScale: 1.35, damageBonus: 1, speedBonus: -0.8, pierceBonus: 0, radius: 0.34, life: 1.18, color: palette.lemon };
  }
  ...
  if (weaponId === "signal_pulse") {
    return { label: "signal pulse", range: 7.1, cooldownScale: 1.08, damageScale: 0.58, damageBonus: 1, speedBonus: -1.6, pierceBonus: 0, radius: 0.28, life: 0.9, color: palette.blue };
  }
  ...
  if (weaponId === "redline_suture") {
    return { label: "redline suture", range: 7.8, cooldownScale: 0.94, damageScale: 0.92, damageBonus: 1, speedBonus: 0.8, pierceBonus: 1, radius: 0.21, life: 1.12, color: palette.mint };
  }
  ...
  if (weaponId === "vector_lance") {
    return { label: "vector lance", range: 9.0, cooldownScale: 1.06, damageScale: 0.82, damageBonus: 0, speedBonus: 2.2, pierceBonus: 1, radius: 0.13, life: 1.0, color: palette.white };
  }
  ...
  if (weaponId === "prism_cannon") {
    return { label: "prism cannon", range: 9.2, cooldownScale: 1.08, damageScale: 0.82, damageBonus: 0, speedBonus: 2.5, pierceBonus: 2, radius: 0.13, life: 1.12, color: palette.white };
  }
